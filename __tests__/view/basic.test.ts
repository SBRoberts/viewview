...
describe("Basic View Rendering", () => {
  ...
  test("should render a static view with a ref", () => {
    ...
    const element = view`<h1 ref="${ref}" data-testid="${TEST_ID}">${text}</h1>`;
  ...
  test("should render a static view with an array argument", () => {
    ...
    const listItemViews = view`${listItemText.map(
      (item) => view`<li data-testid="${TEST_ID}">${item}</li>`
    )}`;

    const listView = view`
      <ul ref="${ref}">${listItemViews}</ul>`;
    ...
    const collectedElements = listView.collect();
  });
  ...
  test("should collect all elements with a ref attribute", () => {
    ...
    const collectedElements = sectionView.collect();

    const sectionEl = getByTestId(document.body, TEST_ID);

    // Assert that all of our refs are collected and actually on the DOM
    for (let ref in refs) {
      const el = collectedElements[ref];
      expect(collectedElements).toHaveProperty(ref);
  ...
  test("should render a variety of argument types", () => {
    const generateChildEl = (index = 0, ref = "") =>
      view`<p ref="${ref}" data-testid="generatedEl">This is element number ${index} </p>`;
    ...
    Object.keys(data).forEach((ref) => {
      ...
      if (Array.isArray(prop)) {
        prop.forEach((item, index) => {
          ...
          const el = collectedElements[derivedRef];
          expect(collectedElements).toHaveProperty(derivedRef);
          expect(el).toBeInTheDocument();
        });
      } else if (prop) {
        const el = collectedElements[ref];
